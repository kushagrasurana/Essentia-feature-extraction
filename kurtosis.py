...
def generate_kurtosis(file, file_name, path):
	...
	for frame in FrameGenerator(audio, frameSize = 2048, hopSize = 512):
        ...
        pool.add('lowlevel.kurtosis', skewness)

	output = YamlOutput(filename = join(path, 'kurtosis', str(file_name) + '.json')
			 , format = 'json')
	output(pool)
...
def main():
	...
	for file, file_name in zip(files, file_names):
		generate_kurtosis(file, file_name, mypath)
